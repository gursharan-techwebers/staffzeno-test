import AuthPageTransition from "@/components/shared/AuthPageTransition";
import Image from "next/image";
import Link from "next/link";

const AuthLayout = ({ children }: { children: React.ReactNode }) => {
  return (
    <div className="grid min-h-svh lg:grid-cols-2 py-15 md:py-0">
      <div className="relative flex flex-col gap-4 px-6 md:px-10 py-20 md:py-26">
        <div className="absolute top-6 left-1/2 -translate-x-1/2 md:top-10 md:left-10 md:translate-x-0">
          <Link href="/" className="flex items-center gap-2 font-medium">
            <Image
              src="/logos/primary/staffzeno_primary_logo_dual_tone.svg"
              alt="StaffZeno"
              width={200}
              height={200}
              className="h-auto w-30 md:w-33"
            />
          </Link>
        </div>

        <div className="flex flex-1 items-center justify-center mt-8 md:mt-6">
          <div className="w-full max-w-sm">
            <AuthPageTransition>{children}</AuthPageTransition>
          </div>
        </div>
      </div>

      <div className="relative hidden bg-muted lg:block">
        <Image
          src="/placeholder.svg"
          alt="StaffZeno authentication background"
          className="absolute inset-0 h-full w-full object-cover dark:brightness-[0.2] dark:grayscale"
          height={1080}
          width={1920}
          loading="eager"
        />
      </div>
    </div>
  );
};

export default AuthLayout;
